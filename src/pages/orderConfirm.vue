...
export default {
    ...
    data() {
        return {
            ...
            showEditModal: false, //是否显示新增或则编辑弹窗
    ...
    methods: {
        ...
        submitAddress() {
            ...
            if (userAction == 0 || userAction == 1) {
                ...
                if (!receiverName) errMsg = '请输入收货人名称';
                    ...
                    errMsg = '请输入正确格式的手机号';
                else if (!receiverProvince) errMsg = '请选择省份';
                else if (!receiverCity) errMsg = '请选择对应的城市';
                else if (!receiverAddress || !receiverDistrict) errMsg = '请输入收货地址';
                else if (!/\d{6}/.test(receiverZip)) errMsg = '请输入六位邮编';
        ...
        getCartList() {
            this.axios.get('/carts').then(res => {
                let list = res.cartProductVoList; //获取购物车所有商品总数据
                this.cartTotalPrice = res.cartTotalPrice; //商品总金额
                this.cartList = list.filter(item => item.productSelected); //过滤选中状态的商品
